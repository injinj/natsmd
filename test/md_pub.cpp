...
struct NatsDataCallback : public EvConnectionNotify, public NatsClientCB,
  ...
  EvNatsClient & client;          /* connection to nats */
  ...
  NatsDataCallback( EvPoll &p,  EvNatsClient &c,  const char **s,  size_t n,
...
void
NatsDataCallback::init_subjects( void ) noexcept
{
  size_t len = 0;
...
void
NatsDataCallback::on_write_ready( void ) noexcept
{
  this->run_publishers();
}
...
NatsDataCallback::send_dict_request( void ) noexcept
{
  ...
  EvPublish pub( DICT_SUBJ, DICT_SUBJ_LEN, inbox, inbox_len,
                 "x", 1, this->client.sub_route, 0, 0, 0, 0 );
...
main( int argc, const char *argv[] )
{
 ...
 "%s [-d daemon] [-n network] [-s service] [-c cfile_path] [-x] [-e] subject ...\n"
  ...
  NatsDataCallback       data( poll, conn, &argv[ first_sub ], n,
                               sub_cnt, pub_cnt,
